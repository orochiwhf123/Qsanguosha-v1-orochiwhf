...
---------------
--回合开始时，你可以令你距离1以内的角色各弃置一张牌（包括装备），然后你将其中一张牌交给一名其他角色，从剩余弃牌堆中获得至多三张牌（此为注释部分，实际效果改为任意分配）。
lundao=sgs.CreateTriggerSkill{
	...
	on_trigger=function(self,event,player,data)
		...
		for _,f in sgs.qlist(room:getAlivePlayers()) do
			...
			if player:distanceTo(f) < 2 and not f:isNude() then
			    pl:append(f)
			end
			end
