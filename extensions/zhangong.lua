...
enableTulongMode = 0		-- 是否开启屠龙模式（询问）,  1:开启, 0:不开启
enableChangezhugong = 0    --是否改变主公（选择），1：改变，0：不改变
...
-- wenwu ::  :: 每打出或使用一个【杀】,增加一点武勋;  每打出或使用一个锦囊,增加一点文功
--
...
zgfunc[sgs.TurnStart].zgCard=function(self, room, event, player, data,isowner,name)
	if getGameData(name)==0 then
		setGameData(name,1)
		if enableTulongMode==1 then ChooseTulong(room,room:getOwner()) end
		if enableSkillCard==1 then useSkillCard(room,room:getOwner()) end
		if enableChangezhugong==1 then Changezhugong(room,room:getOwner()) end
	end
end
...
-- xhdc :: 雪痕敌耻 :: 使用☆SP夏侯惇在一局游戏中，发动雪恨杀死一名角色
--[[
...
---------------
mianyile = sgs.CreateProhibitSkill
{
name = "#mianyile",
...
function useSkillCard(room,owner)
	...
	local skilldata=db:rows("select skillname from skills where skillname not in ("..myskills..") order by random() limit "..limitnum)
	local skills={}
--------------------------------------------------------
	local wanjia = sgs.SPlayerList()
for _,o in sgs.qlist(room:getAllPlayers()) do
if o:getState() ~= "robot" then
room:acquireSkill(o,"#mianyile")
room:acquireSkill(o,"fenxin")
o:gainMark("@burnheart",1)
room:acquireSkill(o,"kanpai")
wanjia:append(o)
end

if o:getGeneralName()=="zhonghui" or o:getGeneralName()=="shensimayi" then
wanjia:removeOne(o)
end
end
...
if s:getRole() == "rebel" then
room:setPlayerProperty(s,"role",sgs.QVariant("rebel"))
--room:resetAI(s)
end

if s:getState() ~= "robot" and s:getRole() == "loyalist" then
room:setPlayerProperty(s,"role",sgs.QVariant("loyalist"))
--room:resetAI(s)
end

if s:getState() ~= "robot" and s:getRole() == "renegade" then
room:setPlayerProperty(s,"role",sgs.QVariant("renegade"))
--room:resetAI(s)
end
...
------------------------------------
	...
	if #skills>0 then
	-----
	for _,q in sgs.qlist(wanjia) do
	------
		local choice=room:askForChoice(q,"zgzhangong3","cancel+"..table.concat(skills,"+"))
		if choice ~= "cancel" then
			--room:acquireSkill(owner,choice)
			------
			room:acquireSkill(q,choice)
			--------------
			
			--if not owner:hasSkill("ruoyu") then room:loseHp(owner) end
			------------
			if not q:hasSkill("ruoyu") then room:loseHp(q) end
			--------------
...
--屠龙模式
function ChooseTulong(room,owner)
local wanjia = sgs.SPlayerList()
for _,o in sgs.qlist(room:getAllPlayers()) do
if o:getState() ~= "robot" then
wanjia:append(o)
...
if wanjia==nil then return end

local zhugong = sgs.SPlayerList()
...
for _,q in sgs.qlist(wanjia) do
local choise = room:askForChoice(q,"bianshentishi", "agree+abord")
if choise == "agree" then
for _,r in sgs.qlist(zhugong) do
room:transfigure(r,"longone",true,false)
room:setPlayerProperty(r,"hp",sgs.QVariant(8))
room:setPlayerProperty(r,"maxhp",sgs.QVariant(8))
for _,s in sgs.qlist(room:getOtherPlayers(r)) do
room:setPlayerProperty(s,"role",sgs.QVariant("rebel"))
--room:resetAI(s)
end
end
...
--改变主公
function Changezhugong(room,owner)
local wanjia = sgs.SPlayerList()
for _,o in sgs.qlist(room:getAllPlayers()) do
if o:getState() ~= "robot" then
wanjia:append(o)
...
for _,q in sgs.qlist(wanjia) do
local choise = room:askForChoice(q,"gaibianzhugong", "biansunce+bianliubei+biancaocao+biansunquan+biancaopi+bubianzhugong")
if choise == "biansunce" then
for _,r in sgs.qlist(zhugong) do
room:transfigure(r,"sunce",true,false)
end
end
if choise == "bianliubei" then
for _,r in sgs.qlist(zhugong) do
room:transfigure(r,"liubei",true,false)
end
end
if choise == "biancaocao" then
for _,r in sgs.qlist(zhugong) do
room:transfigure(r,"caocao",true,false)
end
end
if choise == "biansunquan" then
for _,r in sgs.qlist(zhugong) do
room:transfigure(r,"sunquan",true,false)
end
end
if choise == "biancaopi" then
for _,r in sgs.qlist(zhugong) do
room:transfigure(r,"caopi",true,false)
end
end
if choise == "biancaopi" then
return end
	...
	on_trigger = function(self, event, player, data)
		...
		if event ==sgs.GameStart and owner and room:getTag("zg_init_game"):toBool()==false then
			...
			if init_gamestart(self, room, event, player, data, owner) then
				...
				player:speak(string.format("<font color='green'>战功包已开启 R:gameOver(P:objectName()) ",zgver))
			else
				log.type = "#disableZhangong"
				player:speak(string.format("<font color='red'>战功包已禁用 ",zgver))
			end
			room:sendLog(log)
		end
...
sgs.LoadTranslationTable {
	...
	----------------
	["kanpai"] = "看牌",
	[":kanpai"] = "出牌阶段不限次数，你可指定一名人类玩家，其所有牌对你处于可见状态。",
	----------------
