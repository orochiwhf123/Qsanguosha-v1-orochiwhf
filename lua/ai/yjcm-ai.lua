...
xinzhan_skill.getTurnUseCard=function(self)
	if not self.player:hasUsed("XinzhanCard") and self.player:getHandcardNum() >= 3 then
...
function sgs.ai_slash_prohibit.huilei(self, to)
	if self:hasSkills("qianxi|jueqing") then return false end
...
sgs.ai_skill_invoke.enyuan = function(self, data)
	...
	if from then
		...
		if self.xhtarget then
			return self:isFriend(self.xhtarget)
		else
...
xianzhen_skill.getTurnUseCard=function(self)
	...
	elseif self.player:hasUsed("XianzhenCard") and (self.player:hasFlag("xianzhen_success") or self.player:hasFlag("xianzhen_failed")) then
