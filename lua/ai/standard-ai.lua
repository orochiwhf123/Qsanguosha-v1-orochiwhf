...
sgs.ai_skill_choice.jianxiong = function(self, choice)
	return "use"
end

sgs.ai_skill_playerchosen.jianxionguse = function(self, targets)
...
sgs.ai_skill_use_func.JijiangCard=function(card,use,self)
	...
	for _, enemy in ipairs(self.enemies) do
		if (self.player:canSlash(enemy, not no_distance) and not self:cantbeHurt(enemy) and
