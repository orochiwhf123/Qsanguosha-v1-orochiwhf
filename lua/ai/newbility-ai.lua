...
diezhi_skill.getTurnUseCard = function(self)
	...
	return sgs.Card_Parse("#diezhi:.")
end
sgs.ai_skill_use_func["#diezhi"] = function(card, use, self)
	use.card = card
end
sgs.ai_skill_use_func["#stink"] = function(card, use, self)
	use.card = card
end
...
sgs.ai_skill_choice["magicwand"] = function(self, choices)
	local players = self.room:getOtherPlayers(self.player)
	players = sgs.QList2Table(players)
	local kanze
	for _, other in ipairs(players) do
		if other:hasSkill("guhuo") then kanze = other break end
	end
			local r = math.random(0, 1)
		if self:isEnemy(kanze) and self.player:getHp()>1 and r==0 then
			return "question"
		else
			return "noquestion"
	end
