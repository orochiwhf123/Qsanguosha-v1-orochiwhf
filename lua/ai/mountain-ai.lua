...
--制霸拿牌
sgs.ai_skill_invoke.zhiba = true
...
function sgs.ai_skill_choice.huashen(self, choices)
	...
	if self.player:getPhase() == sgs.Player_NotActive then
		if self.player:getHp() == 1 then
			...
			for _, askill in ipairs(("wuhun|duanchang|jijiu|longhun|jiushi|jiuchi|buyi|huilei|dushi|juejing|zhuiyi|nosjiefan"):split("|")) do
		...
		"jiushi|qixi|luoyi|wenjiu|jiuchi|longhun|wusheng|wushen|longdan|nosgongqi|lihuo|shensu|jiangchi|lianhuan|yinghun|houyuan|jujian|huoji|luanji|gongmou|" ..
