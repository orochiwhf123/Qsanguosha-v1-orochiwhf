...
public:
    ...
    virtual int getDrawNum(ServerPlayer *liubiao, int n) const{
        ...
        QSet<QString> kingdom_set;
        if(liubiao->parent()){
            foreach(const Player *player, liubiao->parent()->findChildren<const Player *>()){
                if(player->isAlive()){
...
public:
    ...
    virtual bool trigger(TriggerEvent, Room* room, ServerPlayer *player, QVariant &data) const{
            ...
            else
        {
            ...
            {
                room->recover(player, RecoverStruct(), true);
...
void AnxuCard::use(Room *room, ServerPlayer *, const QList<ServerPlayer *> &targets) const{
...
public:
    ...
    virtual bool trigger(TriggerEvent event, Room* room, ServerPlayer *, QVariant &data) const{
        ...
        if(event == PhaseChange){
            if(chengpu->getPhase() == Player::Discard && !chengpu->isKongcheng() &&
                chengpu->getPile("wine").isEmpty())
                room->askForUseCard(chengpu, "@@chunlao", "@chunlao");
        }else if(event == Dying){
            DyingStruct dying = data.value<DyingStruct>();
			if(!dying.savers.contains(chengpu))
				return false;
