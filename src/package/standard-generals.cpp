...
public:
    ...
    virtual void onDamaged(ServerPlayer *caocao, const DamageStruct &damage) const{
        ...
        if(room->askForSkillInvoke(caocao, "jianxiong", data)){
            ...
            if (card->isAvailable(caocao)||!caocao->isProhibited(player, card))//这段不检测杀
			players << player;

            if (card->isKindOf("Slash") && (player->isKongcheng() && player->hasSkill("kongcheng")))//检测杀专用
            players.removeOne(player);
			}

			if (players.length() == 0||card->isVirtualCard()){
			caocao->drawCards(1);
			}
			else{
            QString choice = room->askForChoice(caocao, objectName(), "use+notuse2");
			if(choice == "use"){
			    CardUseStruct use;
                use.card = card;
                use.from = caocao;
                use.to << room->askForPlayerChosen(caocao, players, "jianxionguse");
                room->useCard(use);}

            //else if(choice == "notuse1"){
               //room->setPlayerProperty(caocao, "maxhp", caocao->getMaxHP() + 1);
               //QList<ServerPlayer *> players = room->getAllPlayers();
               //RecoverStruct recover;
               //recover.who = caocao;
               //room->recover(room->askForPlayerChosen(caocao, players, "huixuetip"), recover);
            ...
            caocao->drawCards(1);
                //RecoverStruct recover;
                //recover.who = caocao;
                //room->recover(caocao, recover);
                //QList<ServerPlayer *> players = room->getAllPlayers();
                //room->loseMaxHp(room->askForPlayerChosen(caocao, players, "maxhplost"));
...
public:
    ...
    virtual bool viewFilter(const CardItem *to_select) const{
		...
		return (to_select->getFilteredCard()->isBlack() or to_select->getFilteredCard()->isRed());
    }
...
class Kurouko: public PhaseChangeSkill{
public:
    Kurouko():PhaseChangeSkill("#kurouko"){
    }
    ...
    virtual bool onPhaseChange(ServerPlayer *huanggai) const{
        if(huanggai->getPhase() == Player::Start){
            huanggai->setFlags("-kurou_used");
        }
...
class Kuroukeep: public MaxCardsSkill{
public:
    Kuroukeep():MaxCardsSkill("#kuroukeep"){
    }

    virtual int getExtra(const Player *target) const{
        if(target->hasSkill(objectName()) && target->hasFlag("kurou_used") )
            return 3;
        else
            return 0;
    }
...
void StandardPackage::addGenerals(){
    ...
    huanggai->addSkill(new Kurouko);
    huanggai->addSkill(new Kuroukeep);
    related_skills.insertMulti("kurou", "#kurouko");
    related_skills.insertMulti("kurou", "#kuroukeep");
...
class Kanpai:public ZeroCardViewAsSkill{
public:
    Kanpai():ZeroCardViewAsSkill("kanpai"){
