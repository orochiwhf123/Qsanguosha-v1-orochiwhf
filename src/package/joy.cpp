...
//////////////////////////////////////////////////////七星刀
class QxKnifeSkill: public WeaponSkill{
public:
    QxKnifeSkill():WeaponSkill("qx_knife"){
        events << Predamage;
    }

    virtual bool trigger(TriggerEvent, Room* room, ServerPlayer *player, QVariant &data) const{
      DamageStruct damage = data.value<DamageStruct>();
      PlayerStar to = damage.to;
        if(damage.card && damage.card->inherits("Slash") && room->askForSkillInvoke(player, objectName(), data)){

            QString choice = room->askForChoice(player, objectName(), "geidao+shashang");
            if(choice == "geidao"){
                room->moveCardTo(player->getWeapon(), damage.to, Player::Hand);
                damage.to->drawCards(1);}

            else
                {
                    LogMessage log;
                    log.type = "#QxKnifeEffect";
                    log.from = player;
                    log.to << damage.to;
                    log.arg = QString::number(damage.to->getLostHp());
                    room->sendLog(log);
                    ...
                    damage.damage = damage.to->getLostHp();
                    room->damage(damage);
                }
...
QxKnife::QxKnife(Suit suit, int number)
    :Weapon(suit, number, 2)
{
    setObjectName("qx_knife");
    skill = new QxKnifeSkill;
}
////////////////////////////////////////////////////////七星刀
...
#include "client.h"
#include "carditem.h"
class HxjSkill: public ArmorSkill{
public:
    HxjSkill():ArmorSkill("hxj"){
        events << Predamaged  << CardEffected;
        //events << Predamaged;
    }
    ...
    virtual bool trigger(TriggerEvent event, Room* room, ServerPlayer *player, QVariant &data) const{
        if(event == Predamaged){
            ...
            /*if(damage.nature == DamageStruct::Normal && (damage.from->getGeneral()->isMale() || damage.from->getGeneral()->isFemale()) && player->distanceTo(damage.from) <= 1 ){
                //room->setEmotion(player, "armor/vine");
                LogMessage log;
                log.from = player;
                log.type = "#HxjDistanceEffect";
                log.arg = objectName();
                room->sendLog(log);
...
Hxj::Hxj(Suit suit, int number):Armor(suit, number){
    setObjectName("hxj");
    skill = new HxjSkill;
}
    ...
    :Package("joy_equip")
{
    ...
    (new QxKnife(Card::Spade, 7))->setParent(this);
    (new Hxj(Card::Heart, 3))->setParent(this);
