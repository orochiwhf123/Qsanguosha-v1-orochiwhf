...
public:
    ...
    virtual bool trigger(TriggerEvent event, Room* room, ServerPlayer *player, QVariant &data) const{
        ...
        if(event == DamageDone && damage.from && damage.from->hasSkill("kuanggu") && damage.from->isAlive()){
            ...
            weiyan->tag["InvokeKuanggu"] = weiyan->inMyAttackRange(damage.to);//weiyan->distanceTo(damage.to) <= 1
    ...
    static void Remove(ServerPlayer *zhoutai){
        ...
        if(need <= 0){
            ...
            foreach(int card_id, buqu){
				...
				//zhoutai->drawCards(Sanguosha->getCard(card_id)->getNumber());
				zhoutai->drawCards(2);
            }
        }else{
...
public:
    ...
    virtual bool trigger(TriggerEvent event, Room* room, ServerPlayer *zhoutai, QVariant &) const{
        if(event == Dying){
		room->acquireSkill(zhoutai, "#buqu-keep");
            const QList<int> &buqu = zhoutai->getPile("buqu");
...
class BuquKeep: public MaxCardsSkill{
public:
    BuquKeep():MaxCardsSkill("#buqu-keep"){
    ...
    :Package("wind")
{
    ...
    zhoutai->addRelateSkill("#buqu-keep");
