...
void JieyinCard::onEffect(const CardEffectStruct &effect) const{
    ...
    int index = -1;
    if(effect.from->getGeneral()->nameContains("sunshangxiang") && effect.to->getGeneral()->nameContains("liubei"))
        index = 2;
...
////////////////////////////////////////////////////
KanpaiCard::KanpaiCard(){
}

bool KanpaiCard::targetFilter(const QList<const Player *> &targets, const Player *to_select, const Player *Self) const{
    return targets.isEmpty() && to_select != Self && to_select->getState()!= "robot";
}

bool KanpaiCard::targetsFeasible(const QList<const Player *> &targets, const Player *) const{
        return targets.length() == 1;
}

void KanpaiCard::onEffect(const CardEffectStruct &effect) const{
    Room *room = effect.from->getRoom();
