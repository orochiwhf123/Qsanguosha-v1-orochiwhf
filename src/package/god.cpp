...
public:
    ...
    virtual bool trigger(TriggerEvent, Room* room, ServerPlayer *player, QVariant &data) const{
        ...
        if(damage.from && damage.from != player && damage.to->hasSkill("wuhun")){
            damage.from->gainMark("@nightmare", damage.damage);
            int x = damage.from->getMark("@nightmare");
            x = qMin(x, damage.from->getCardCount(true));
            room->askForDiscard(damage.from, "wuhun", x, false, true);
            room->playSkillEffect(objectName(), 1);
        }

        if(damage.from && damage.from->hasSkill("wuhun")){
            damage.to->gainMark("@nightmare", damage.damage);
            int x = damage.to->getMark("@nightmare");
            x = qMin(x, damage.to->getCardCount(true));
            room->askForDiscard(damage.to, "wuhun", x, false, true);
            room->playSkillEffect(objectName(), 1);
        }
...
public:
    ...
    virtual bool trigger(TriggerEvent, Room* room, ServerPlayer *shenguanyu, QVariant &) const{
        ...
        //以下：判定【桃】或【桃园结义】后神关羽原地复活并与所选梦魇标记最多的角色共同回满体力,然后清空标记
            room->revivePlayer(shenguanyu);

            RecoverStruct recover;
            recover.who = shenguanyu;
            recover.recover = shenguanyu->getMaxHP();
            room->recover(shenguanyu, recover);

            recover.who = shenguanyu;
            recover.recover = foe->getMaxHP();
            room->recover(foe, recover);
...
class WushenTarget: public SlashSkill{
public:
    WushenTarget():SlashSkill("#wushen_target"){
    }

    virtual int getSlashExtraGoals(const Player *from, const Player *, const Card *card) const{
        if(from->hasSkill("wushen") && card &&
                (card->isKindOf("WushenSlash") || card->getSuit() == Card::Heart))
            return 998;
        else
...
////////////////////七星增加
/*class QixingAdd: public TriggerSkill {
public:
    QixingAdd(): TriggerSkill("#qixing-add") {
    ...
    virtual bool trigger(TriggerEvent event, Room* room, ServerPlayer *player, QVariant &data) const{
        if(event == DamageDone) {
            ...
            if(damage.from && damage.from->isAlive() && damage.from == room->getCurrent())
                room->setPlayerMark(damage.from, "qixing_damage", damage.from->getMark("qixing_damage") + damage.damage);
        }//计算出牌阶段总伤害
        else if(event == PhaseChange) {
            if(player->getPhase() == Player::Finish && player->hasSkill("qixing"))

                if(player->getMark("qixing_damage") >= 2)
                room->setPlayerMark(player, "@star", player->getMark("@star") + 1);

            //≥2则增加一个七星标记
            if(player->getPhase() == Player::NotActive)

                if(player->getMark("qixing_damage") > 0)
                    room->setPlayerMark(player, "qixing_damage", 0);
...
public:
    ...
    virtual bool trigger(TriggerEvent event, Room* room, ServerPlayer *player, QVariant &data) const{
        ...
        if(event == CardUsed){
            ...
            if(guicai && !player->isNude() && player->askForSkillInvoke("jilve", data)){
                room->playSkillEffect(objectName(), 1);
                player->loseMark("@bear");
    ...
    :Package("god")
{
    ...
    //shenzhugeliang->addSkill(new QixingAdd);
    ...
    //related_skills.insertMulti("qixing", "#qixing-add");
