...
public:
    ...
    virtual bool trigger(TriggerEvent, Room* room, ServerPlayer *player, QVariant &data) const{
        ...
        if(killer && damage->from->getGeneralName() != "anjiang"){
            ...
            return false;
            if (damage->from->getGeneralName() == "longone" || damage->from->getGeneral2Name() == "longone")
            return false;
...
if(can_invoke){
        ...
        choicelist << "draw";
        if (sunce->getLostHp() != 0)
            choicelist << "recover";
        QString choice;
        if (choicelist.length() >=2)
            choice = room->askForChoice(sunce, objectName(), choicelist.join("+"));
        else
            choice = "draw";
        if(choice == "recover"){
            RecoverStruct recover;
            recover.who = sunce;
            room->recover(sunce, recover);
        }else
            room->drawCards(sunce, 2);
    ...
    virtual bool trigger(TriggerEvent event, Room* room, ServerPlayer *player, QVariant &data) const{
        if(event == GameStart){
            ...
            if(!pindian->isSuccess()){
                room->playSkillEffect(objectName(), 2);
				if(!pindian->to->askForSkillInvoke("zhiba"))
				return false;
...
void TiaoxinCard::onEffect(const CardEffectStruct &effect) const{
    ...
    if(slash){
        ...
        use.from = effect.to;
        room->useCard(use);
    }else if(!effect.to->isAllNude()){
        room->obtainCard(effect.from,room->askForCardChosen(effect.from, effect.to, "hej", "tiaoxin"));
    }
}
...
public:
    ...
    virtual bool onPhaseChange(ServerPlayer *player) const{
        ...
        if(erzhang->askForSkillInvoke("guzheng", cards.length())){
            ...
            int to_back = room->askForAG(player, cards, false, objectName());
    ...
    virtual bool trigger(TriggerEvent event, Room* room, ServerPlayer *liushan, QVariant &data) const{
        ...
        if(event == CardEffected){
            ...
            if(!room->askForCard(effect.from, "BasicCard", "@xiangle-discard", data, CardDiscarded))
                ...
                room->setPlayerMark(effect.from, "xiangle", 1);
        }
        else if(event == CardFinished){
            //room->setPlayerFlag(liushan, "-xiangle_invoke");
            CardUseStruct use = data.value<CardUseStruct>();
            room->setPlayerMark(use.from, "xiangle", 0);}
        else{
            //return liushan->hasFlag("xiangle_invoke");
            SlashEffectStruct effect = data.value<SlashEffectStruct>();
            if (effect.from->getMark("xiangle") > 0){
            room->setPlayerMark(effect.from, "xiangle", 0);
            return true;}
        }
...
public:
    ...
    virtual bool onPhaseChange(ServerPlayer *liushan) const{
        ...
        if(can_invoke){
			...
			room->playLightbox(liushan, "ruoyu", "3000", 3000);
    ...
    static QStringList GetAvailableGenerals(ServerPlayer *zuoci){
        ...
        if(banned.isEmpty()){
            banned << "zuoci" << "zuocif" << "guzhielai" << "dengshizai" << "caochong"
                   << "jiangboyue" << "shenzhugeliang" << "shenlvbu" << "zhugejin"
                   << "bgm_lvmeng" << "bgm_pangtong";
        }
