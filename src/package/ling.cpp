...
//单独写效果，爆衣BUFF可与原版裸衣BUFF叠加
class BaoyiBuff: public TriggerSkill{
public:
    BaoyiBuff():TriggerSkill("#baoyi"){
        events << Predamage;
    }

    virtual bool triggerable(const ServerPlayer *target) const{
        return target->hasFlag("baoyi") && target->isAlive();
    }
...
public:
    ...
    virtual bool trigger(TriggerEvent, Room* room, ServerPlayer *player, QVariant &data) const{
        ...
        if(damage.card && damage.card->isRed() && !damage.to->isAllNude() && player->askForSkillInvoke(objectName(), data)){
...
public:
    ...
    virtual bool onPhaseChange(ServerPlayer *gongsun) const{
        ...
        if(gongsun->getPhase() == Player::Finish && gongsun->askForSkillInvoke(objectName())){
            ...
            if(gongsun->isNude() || !room->askForCard(gongsun, "EquipCard,TrickCard", "@zhulou-discard", QVariant(), CardDiscarded))
                room->loseHp(gongsun);
        }
        return false;
    }
    ...
    :Package("ling")
{
    ...
    neo_xuchu->addSkill(new BaoyiBuff);
    //related_skills.insertMulti("neoluoyi", "baoyi");
