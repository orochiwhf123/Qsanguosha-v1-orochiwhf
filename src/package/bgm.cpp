...
public:
    ...
    virtual bool trigger(TriggerEvent event, Room* room, ServerPlayer *player, QVariant &data) const{
        if(event == PhaseChange && player->getMark("@hate") > 0 && player->hasLordSkill(objectName())
            && player->getPhase() == Player::RoundStart && player->getCards("he").length() > 1){
    ...
    virtual bool onPhaseChange(ServerPlayer *player) const{
        ...
        if(player->getPhase() == Player::Finish && xiahou->getMark("@fenyong") > 0){
            ...
            if (choice == "slash") {
                ...
                room->playSkillEffect(objectName(), 1);
                int cardnum = player->getHandcardNum()+player->getCards("e").length();
                if (cardnum > xiahou->getLostHp())
                {DummyCard *dummy = room->getCardsOnetime(xiahou, player, xiahou->getLostHp(), objectName());
                room->throwCard(dummy, player, xiahou);}
                else
                {DummyCard *dummy = room->getCardsOnetime(xiahou, player, cardnum, objectName());
                room->throwCard(dummy, player, xiahou);
                delete dummy;}
            }
        }
...
BGMPackage::BGMPackage():Package("BGM"){
    General *bgm_zhaoyun = new General(this, "bgm_zhaoyun", "qun", 4);
