...
void XuanhuoCard::onEffect(const CardEffectStruct &effect) const{
    ...
    bool flag = false;
    if(!targets.isEmpty()){
        ServerPlayer *victim = room->askForPlayerChosen(effect.from, targets, skill_name);
...
public:
    ...
    virtual bool trigger(TriggerEvent, Room* room, ServerPlayer *player, QVariant &data) const{
        ...
        if (damage.to->isNude())
            return false;
            //room->moveCardTo(Sanguosha->getCard(room->askForCardChosen(damage.to,damage.to, "he", "pojungeipai")), player, Player::Hand,room->getCardPlace(room->askForCardChosen(damage.to,damage.to, "he", "pojungeipai")) != Player::Hand);
            //room->moveCardTo(Sanguosha->getCard(room->askForCardChosen(player,player, "he", "pojungeipai")), damage.to, Player::Hand,room->getCardPlace(room->askForCardChosen(player,player, "he", "pojungeipai")) != Player::Hand);
//以上一句话写入是错误的，要先定义选择的牌，否则会二次询问。

        int card_id =room->askForCardChosen(damage.to,damage.to, "he", "pojungeipai");
        room->obtainCard(player, Sanguosha->getCard(card_id),true);
        int card_idd =room->askForCardChosen(player,player, "he", "pojungeipai");
        room->obtainCard(damage.to, Sanguosha->getCard(card_idd),true);
        //不隐藏，明牌交换（用moveCardTo也可以，true就行），与张绣【从谏】的暗换区分开来。
//moveCardTo可以把牌移动到各种区域，比如手牌扔判定区，obtainCard只能是获得到手牌区。
        }
...
public:
    ...
    virtual bool triggerable(const ServerPlayer *target) const{
        return TriggerSkill::triggerable(target) && target->getCards("e").length() > 1;
    }

    virtual bool trigger(TriggerEvent , Room* room, ServerPlayer *player, QVariant &data) const{
        DamageStruct damage = data.value<DamageStruct>();
        if (damage.from && ((damage.from->getHandcardNum() > player->getHandcardNum()) or
                (damage.from->getHp() > player->getHp())))
...
void XianzhenCard::onEffect(const CardEffectStruct &effect) const{
    ...
    if(effect.from->pindian(effect.to, "xianzhen", card)){
        ...
        room->setFixedDistance(effect.from, effect.to, 1);//依然生效
        room->setPlayerFlag(effect.to, "wuqian");//依然生效
...
public:
    ...
    virtual bool isEnabledAtPlay(const Player *player) const{
        return ! player->hasUsed("XianzhenCard") || player->hasFlag("xianzhen_success") || player->hasFlag("xianzhen_failed");//无论输赢均能出杀
    }
    ...
    virtual const Card *viewAs(const QList<CardItem *> &cards) const{
        if(! Self->hasUsed("XianzhenCard")){
            ...
            return card;
        }else if(Self->hasFlag("xianzhen_success") || Self->hasFlag("xianzhen_failed")){
            if(!cards.isEmpty())
                return NULL;
...
public:
    ...
    virtual bool trigger(TriggerEvent, Room* room, ServerPlayer *player, QVariant &data) const{
		...
		if(!dying.savers.contains(wuguotai))
				return false;
...
void XinzhanCard::use(Room *room, ServerPlayer *source, const QList<ServerPlayer *> &) const{
    ...
    foreach(int card_id, cards){
        ...
        if((card->getSuit() == Card::Heart) or (card->getSuit() == Card::Diamond))
            hearts << card_id;
    }
...
public:
    ...
    virtual bool isEnabledAtPlay(const Player *player) const{
		...
		return ! player->hasUsed("XinzhanCard") && player->getHandcardNum() >= 3;
    }
...
class Quanjidistance: public DistanceSkill{
public:
    Quanjidistance():DistanceSkill("#quanjidistance"){

    }

    virtual int getCorrect(const Player *, const Player *to) const{
        if(to->hasSkill(objectName()))
            return +to->getPile("power").length();
        else
            return 0;
    }
};
...
public:
    ...
    virtual bool isEnabledAtPlay(const Player *player) const{
        //return !player->getPile("power").isEmpty();
          return !player->hasUsed("PaiyiCard");
    }
...
public:
    ...
    virtual bool trigger(TriggerEvent event, Room *room, ServerPlayer *zhangchunhua, QVariant &) const{
...
YJCMPackage::YJCMPackage():Package("YJCM"){
    ...
    //zhonghui->addSkill(new Quanjidistance);
