...
void QiangxiCard::onEffect(const CardEffectStruct &effect) const{
    ...
    room->throwCard(effect.to->getWeapon(), effect.to, effect.from);
	room->throwCard(effect.to->getArmor(), effect.to, effect.from);
...
public:
    ...
    virtual bool trigger(TriggerEvent, Room* room, ServerPlayer *pangtong, QVariant &data) const{
        ...
        if(pangtong->askForSkillInvoke(objectName(), data)){
            room->playLightbox(pangtong, objectName(), "", 1500);
...
public:
    ...
    virtual bool triggerable(const ServerPlayer *target) const{
        return TriggerSkill::triggerable(target) && !target->getArmor() && target->getMark("qinggang") == 0 && !target->hasFlag("wuqian");
    }
