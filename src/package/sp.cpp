...
class Guao: public TriggerSkill{
public:
    Guao(): TriggerSkill("guao"){
        events << CardEffected << CardUsed;
    }
        ...
        if(event == CardEffected){
           CardEffectStruct effect = data.value<CardEffectStruct>();
            if(effect.to && effect.to->isAlive()
                && !effect.to->isNude() && effect.card->isBlack()){
                if(room->askForCard(player, ".|.|.|.|red", "@GuaoTip", data, CardDiscarded)){
            ...
            if(use.from && use.from->isAlive()
               && !use.from->isNude() && use.card->isRed())

                if(room->askForCard(player, ".|.|.|.|black", "@GuaoTips", data, CardDiscarded)){
                    ...
                    ServerPlayer *target = room->askForPlayerChosen(player, players, "guao");
                    //ServerPlayer *target = room->askForPlayerChosen(player, room->getOtherPlayers(player), "guao");
                    room->cardEffect(new Duel(Card::NoSuit, 0), player, target);
                }
        }
...
bool SongciCard::targetFilter(const QList<const Player *> &, const Player *to_select, const Player *) const{
...
public:
    ...
    virtual bool onPhaseChange(ServerPlayer *hanba) const{
        ...
        room->playLightbox(hanba, "zhiri", "4000", 4000);
    ...
    :Package("sp")
{
    ...
    sp_guanyu->addSkill("mashu");
    sp_guanyu->addSkill(new Guao);
