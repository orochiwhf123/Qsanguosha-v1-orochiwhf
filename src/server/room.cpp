...
QList<ServerPlayer *> Room::getAllPlayers(bool include_dead) const{ //current is start
    QList <ServerPlayer *> count_players = include_dead ? players : alive_players;
    if (current == NULL)
...
void Room::detachSkillFromPlayer(ServerPlayer *player, const QString &skill_name, bool showlog){
    if(!player->hasSkill(skill_name))
        return;
    if (player->getGeneralName() == "lingdonglai" || player->getGeneral2Name() == "lingdonglai")
	    return;
	if (player->getGeneralName() == "longone" || player->getGeneral2Name() == "longone")
	    return;	
    ...
    if(skill && skill->isVisible()){
        ...
        if(showlog){
            ...
            thread->trigger(SkillLose, player, data);
        }
    }
}
...
bool Room::isCanceled(const CardEffectStruct &effect){
   ...
   // if(effect.card->isNDTrick() && effect.from->hasSkill("tanhu") && effect.to->hasFlag("TanhuTarget"))
      if(effect.card->isNDTrick() && effect.to->hasFlag("TanhuTarget"))
        return false;
...
void Room::swapPile(){
    ...
    int limit = 6;
    if(mode == "04_1v3")
        limit = Config.BanPackages.contains("maneuvering") ? 3 : 2;
    else if(Config.EnableEndless)
        limit = Config.value("EndlessTimes").toInt();
    else
        limit = Config.value("SwapCount", 6).toInt();
...
void Room::useCard(const CardUseStruct &card_use, bool add_history){
    ...
    if(card_use.from->getPhase() == Player::Play && add_history){
        ...
        if(!slash_record){
            ...
            if(!thread->trigger(CardRecord, card_use.from, data)){ // return true mean not add history
...
void Room::throwCard(const Card *card, ServerPlayer *who, ServerPlayer *thrower){
    ...
    if(who){
        ...
        interrupt = thread->trigger(CardThrow, who, data2);
        if(!interrupt)
            interrupt = thread->trigger(CardDiscard, who, data);
    }
    if(!interrupt){
...
void Room::showAllCards(ServerPlayer *source, bool onebyone){
    if(!onebyone)
        showAllCards(source);
    else{
        QList<int> cards = source->handCards();
        foreach(int card_id, cards){
            showCard(source, card_id);
            thread->delay();
        }
    }
}
